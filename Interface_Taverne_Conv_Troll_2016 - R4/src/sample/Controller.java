...
public class Controller implements Initializable {
    ...
    */
    ...
    private ArrayList<ItemType> itemTypesList = new ArrayList();
    ...
    private ArrayList<ListView> listViewCollection = new ArrayList<>();
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        initMenuListViewColorDisplay(MenuListView);
    ...
    private void updateListView(){
        listItemsMenu.clear();
        itemTypesList.clear();
        int nbTabs = SelectTabPane.getTabs().size();

        for (int i = 1; i <nbTabs; i++){
            SelectTabPane.getTabs().remove(1);
        }

        for(Vendable elem : model.getDao().getDBProduits()) {

            if (items.Menu.class.isInstance(elem)) {
                System.out.println("new menu red : '"+elem.getNom()+"'");
                listItemsMenu.add((items.Menu) elem);
            } else {
                boolean found = false;

                for (ItemType current : itemTypesList) {
                    if (current.getName().equals(elem.getCatNom())) {
                        found = true;
                        current.add(elem);
                    }
                }
                if (!found) {
                    ItemType iT = new ItemType(elem.getCatNom());
                    iT.add(elem);
                    itemTypesList.add(iT);
                    System.out.println("new type of item red : '"+elem.getCatNom()+"' with object '"+elem.getNom()+"'");

                }
            }
        }

        addTab(SelectTabPane, itemTypesList);
    }
    ...
    private void initListViewColorDisplay(ListView... listViews) {
        //Pour chaque ListView, on applique une CellFactory qui va modifier l'id d'une ligne en fonction du stock de l'objet correspondant
        //L'id va correspondre à un certain style CSS
    ...
    private void initMenuListViewColorDisplay(ListView... listViews) {
        //Pour chaque ListView, on applique une CellFactory qui va modifier l'id d'une ligne en fonction du stock de l'objet correspondant
        //L'id va correspondre à un certain style CSS
        for (ListView listView : listViews) {
            listView.setCellFactory(lv -> {
                return new ListCell<items.Menu>() {

                    @Override
                    public void updateItem(items.Menu item, boolean empty) {
                        super.updateItem(item, empty);
                        if (item != null) {
                            if (Integer.parseInt(item.getNombreStock()) <= 0) {
                                setId("Rupture");
    ...
    private void addTab(TabPane tabPane, ItemType... itemTypes){
    ...
    private void addTab(TabPane tabPane, ArrayList<ItemType> itemTypes){
        /*
        Pour chaque ItemType :
            * on créé un nouveau Tab et une nouvelle ListView.
            * on donne le nom du type au nouveau Tab
            * on donne la liste d'objet à la nouvelle ListView
            * on lie la nouvelle ListView au nouveau Tab
            * on ajoute le nouveau Tab au TabPane spécifié
         */
        System.out.println("adding new tabs!!");
        for(ItemType itemType : itemTypes) {
            System.out.println("\t adding tab '"+itemType.getName()+"'");
            Tab tab = new Tab();
            ...
            System.out.println("\t -> size : "+itemType.size());
            for(Object currentItem : itemType) {
                System.out.println("\t\t adding '" + currentItem.toString() + "' to this list");
                list.add(currentItem);
            }

            listView.setItems(list);
            initListViewColorDisplay(listView);
            ...
            listView.setOnMouseClicked(new EventHandler<Event>() {
                ...
                public void handle(Event event) {
                    Platform.runLater(new Runnable() {
                        @Override
                        public void run() {
                            handleAddSelection(listView, false);
                        }
                    });
                }
            });
